update_h=function(initialh, hiter,gam, beta, Sig,X,T){
  ...
  for (i in 2:hiter){
    ...
    if (h2 > 1){h2 = 2-h2}
    ind = which(gam==1)
    sigmabeta1 = get_sigmabeta_from_h(h1,gam,Sig,X,T)
    sigmabeta2 = get_sigmabeta_from_h(h2,gam,Sig,X,T)
    lik1 = sum(dnorm(beta[ind], rep(0,length(ind)), sigmabeta1*diag(Sig)[ind], log=TRUE))
    lik2 = sum(dnorm(beta[ind], rep(0,length(ind)), sigmabeta2*diag(Sig)[ind], log=TRUE))
