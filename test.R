...
Sigma[,,1] = dat$Sigma
sigmabeta[1] = var(dat$beta[dat$gamma==1]/diag(dat$Sigma)[dat$gamma==1])
Vbeta = 0.1
tar = rep(0,niter)
bgiter = 50
...
h = rep(0,niter)
hiter=50
plot(1:niter, rep(0,niter), pch=NA, ylim = c(-7000,-6000))
for (i in 2:niter){
...
datgamma = as.data.frame(gamma[1:300, ])
colnames(datgamma) = paste0("var",0:9)
rownames(datgamma) = paste0("iter",1:300)
...
datbeta = as.data.frame(beta[1:300,])
colnames(datbeta) = paste0("var",0:9)
rownames(datbeta) = paste0("iter",1:300)
datbeta$id = rownames(datbeta)
datbeta$id = factor(datbeta$id, levels=datbeta$id)
datbeta2= melt(datbeta)
datbeta2$xx = rep(1:300, 10)
