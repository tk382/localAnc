...
// [[Rcpp::depends("RcppArmadillo")]]
// [[Rcpp::export]]
arma::rowvec colvarNA(arma::mat Y){
  int T = Y.n_cols;
  arma::rowvec colvar = arma::zeros<arma::rowvec>(T);
  for (int i=0; i < T; ++i){
    arma::vec ycol = Y.col(i);
    arma::uvec finiteind = find_finite(ycol);
    arma::vec yy = ycol(finiteind);
...
// [[Rcpp::depends("RcppArmadillo")]]
// [[Rcpp::export]]
Rcpp::List em_cov_c(arma::mat y,
                   const int maxit){
  int orig_p = y.n_cols;
  arma::rowvec vars = colvarNA(y);
  arma::uvec valid_ind = find(vars>1e-6);
  y = y.cols(valid_ind);
  int p = y.n_cols;
  int n = y.n_rows;
  arma::rowvec means = colmeanNA(y);
...
arma::mat em_with_zero_mean_c(arma::mat y,
  ...
  while (diff>0.001 && it < maxit){
    ...
    Sigma = (y_imputed.t() * y_imputed)/(n-1) + bias/n;
    arma::mat diffmat = (Sigma-oldSigma);
    arma::mat diffsq = diffmat%diffmat;
    diff = accu(diffsq);
    oldSigma = Sigma;
...
arma::vec get_target_c(const arma::vec X,
  ...
  arma::mat sigmamudiag = arma::mat(T,T); sigmamudiag.eye();
  sigmamudiag = sigmamudiag * sigma_mu * sigma_mu;
  M = dmvnrm_arma(mu, arma::zeros<arma::rowvec>(mu.size()), sigmamudiag, true);
...
arma::vec betagam_accept_sw_c(const arma::vec X,
                              ...
                              const int changeind,
                              const int change){
  double newtarget = sum(get_target_c(X,Y,mu,sigmabeta1,inputSigma,gam2,beta2,sigma_mu));
  double oldtarget = sum(get_target_c(X,Y,mu,sigmabeta1,inputSigma,gam1,beta1,sigma_mu));
...
// [[Rcpp::depends("RcppArmadillo")]]
// [[Rcpp::export]]
Rcpp::List update_Sigma_mu_c(const int n,
                         const int nu,
                         const arma::vec X,
                         const arma::vec beta,
                         const arma::mat Phi,
  ...
  arma::cube res = rinvwish_c(1, n+nu, emp*(n-1) + Phi*nu);
  return Rcpp::List::create(
    Rcpp::Named("Sigma") = res.slice(0)
  );
}
...
Rcpp::List update_betagam_sw_c(const arma::vec X,
  ...
  for (int i=0; i<bgiter; ++i){
      ...
      arma::rowvec marcor2 = min(marcor)+max(marcor)-marcor;
      double proposal_iter = R::dnorm(beta1(changeind)-beta2(changeind),0,sqrt(Vbeta),true);
      // cout << "beta1 " << beta1.t() << " gam1 " << gam1.t() << "\n";
      // cout << "beta2 " << beta2.t() << " gam2 " << gam2.t() << "\n";
      arma::vec target1 = get_target_c(X,Y,mu,sigmabeta,Sigma,gam1,beta1,sigma_mu);
      arma::vec target2 = get_target_c(X,Y,mu,sigmabeta,Sigma,gam2,beta2,sigma_mu);
      // cout << "target1" << sum(target1) << "\n";
      // cout << "target2" << sum(target2) << "\n";
      double proposal_ratio = 0;
      if(change==1){
        arma::uvec ind1 = find(gam1==0);
        arma::uvec ind2 = find(gam2==1);
      ...
      // cout << "proposal_ratio " << proposal_ratio << "\n";
      arma::vec A = betagam_accept_sw_c(X,Y,mu,sigmabeta,
                                        Sigma,Vbeta,marcor,sigma_mu,
                                        gam1,beta1,
                                        gam2,beta2,
                                        changeind,change);
      NumericVector check2 = runif(1); double check = check2(0);
      if(exp(A(0)) > check){
        // cout << A << "\n";
        // cout << beta2 << "\n";
...
// [[Rcpp::depends("RcppArmadillo")]]
// [[Rcpp::export]]
arma::rowvec update_mu(const arma::vec X,
                    const arma::mat Y,
                    const arma::vec beta,
                    const double sigma_mu){
  int T = Y.n_cols;
  ...
  beta2.col(0) = beta;
  arma::mat r = Y - X2 * beta2.t();
  arma::rowvec muhat = colmeanNA(r);
  //cout << muhat << "\n";
  Rcpp::List em = em_cov_c(r,100);
  arma::mat sigmahat = as<arma::mat>(em["Sigma"]);
  arma::mat nsigmahatinverse = n*sigmahat.i();
  arma::mat sigmazero(T,T); sigmazero.eye(); //identity matrix
  sigmazero = sigmazero * (sigma_mu*sigma_mu);
  arma::mat sigmazeroinverse(T,T); sigmazeroinverse.eye();
  sigmazeroinverse = sigmazeroinverse / (sigma_mu*sigma_mu);
  arma::mat varterm = (sigmazeroinverse + nsigmahatinverse).i();
  //cout << varterm << "\n";
  arma::rowvec meanterm =  (varterm * nsigmahatinverse * muhat.t()).t();
  //cout << meanterm << "\n";
  arma::mat mu = mvrnormArma(1, meanterm.t(), varterm);
  return mu.row(0);
}
...
Rcpp::List run2chains_c(const arma::vec X,
  ...
  smallchange = mean(marcor%marcor) * smallchange;
  ...
  arma::mat tar1 = arma::zeros<arma::mat>(4, niter);
  arma::mat mumat1 = arma::zeros<arma::mat>(niter, T);
  ...
  arma::mat tar2 = arma::zeros<arma::mat>(4, niter);
  arma::mat mumat2 = arma::zeros<arma::mat>(niter, T);
  ...
  mumat1.row(0)      = as<arma::rowvec>(initial_chain1["mu"]);
  ...
  mumat2.row(0)      = as<arma::rowvec>(initial_chain2["mu"]);

  for (int i=1; i<niter; ++i){

   //if(i%50==0){
   //   smallchange = smallchange * 0.8;
   //}
   Rcpp::List bg = update_betagam_sw_c(X,
    ...
    Rcpp::List muSigma1 = update_Sigma_mu_c(n,nu,X,outbeta1.col(i),Phi,Y);
    outSigma1.slice(i) = as<arma::mat>(muSigma1["Sigma"]);
    mumat1.row(i) = update_mu(X,Y,outbeta1.col(i),sigma_mu);
    ...
    Rcpp::List muSigma2 = update_Sigma_mu_c(n,nu,X,outbeta2.col(i),Phi,Y);
    outSigma2.slice(i) = as<arma::mat>(muSigma2["Sigma"]);
    mumat2.row(i) = update_mu(X,Y,outbeta2.col(i), sigma_mu);
    //mumat2.row(i) = as<arma::rowvec>(muSigma2["mu"]);
    hsig = update_h_c(outh2[i-1],
                      hiter,
                      outgam2.col(i),
                      outbeta2.col(i),
                      outSigma2.slice(i),
    ...
    if(i>2*burnin && i%5==0){
      ...
      if(all(rowmean1<0.5) & all(rowmean2<0.5)){
        ...
        mumat1.shed_cols(i+1, niter-1);       mumat2.shed_cols(i+1, niter-1);
        break;
      }else{
        ...
        if(est1.size()==est2.size() && all(est1==est2)){
          ...
          if(diff/est1.size() < 1e-2){
            ...
            outh1.shed_rows(i+1,niter-1);         outh2.shed_rows(i+1, niter-1);
            outsb1.shed_rows(i+1,niter-1);        outsb2.shed_rows(i+1,niter-1);
            tar1.shed_cols(i+1, niter-1);         tar2.shed_cols(i+1, niter-1);
            mumat1.shed_rows(i+1, niter-1);       mumat2.shed_rows(i+1, niter-1);
            break;
          }
        }
      }
...
//       outsb(i) = 1000;
//     }
//     tar.col(i) = get_target_c(X,Y,mu2, outsb(i), Sigma2,gam2, beta2);
//     cout << i << "\n";
//   }
//   return Rcpp::List::create(
...
//   double newtarget = sum(get_target_c(X,Y,mu,sigmabeta1,inputSigma,gam2,beta2));
//   double oldtarget = sum(get_target_c(X,Y,mu,sigmabeta1,inputSigma,gam1,beta1));
...
//   gam2 = gamtemp2;
//   beta2 = betatemp2;
//   double newtarget = sum(get_target_c(X,Y,mu,sigmabeta,Sigma,gam2,beta2));
//   double oldtarget = sum(get_target_c(X,Y,mu,sigmabeta,Sigma,gam1,beta1));
//   double A = newtarget - oldtarget + proposal_ratio;
//   arma::vec check2 = runif(1,0,1); double check = check2(0);
//   if(exp(A) > check){
//     gam1 = gam2;
