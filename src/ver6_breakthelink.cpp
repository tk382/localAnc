...
arma::vec get_target_c(const arma::vec X,
  ...
  if(s>0){
    ...
    for (int j=0; j<s; ++j){
      int newind = ind(j);
      //cout << beta(newind)<< "\n sb" << sqrt(sigmabeta) << "\n";
      //cout << R::dnorm(beta(newind), 0, sqrt(sigmabeta), true) << "\n";
...
Rcpp::List update_h_c(const double initialh,
  ...
  for (int i=1; i<hiter; ++i){
    ...
    for (int j=0; j < ind.size(); ++j){
      int newind = ind(j);
      lik1 = lik1 + R::dnorm(beta(newind), 0, sqrt(sigmabeta1), true);
      lik2 = lik2 + R::dnorm(beta(newind), 0, sqrt(sigmabeta2), true);
    }
...
Rcpp::List run2chains_c(const arma::vec X,
  ...
  // outh1(0)           = get_h_from_sigmabeta_c(X,outsb1(0),
  //                                             outSigma1.slice(0),
  //                                             outgam1.col(0), n, T);
  // outsb1(0)          = sigmabeta;
