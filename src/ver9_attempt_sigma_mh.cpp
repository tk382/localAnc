...
double dmvnrm_arma(const arma::rowvec x,
  ...
  double constants = -(static_cast<double>(xdim)/2.0) * log(M_PI);
...
// [[Rcpp::depends("RcppArmadillo")]]
// [[Rcpp::export]]
double dinvwish_c(const arma::mat W,
                  const int v,
                  const arma::mat S){
  int p = S.n_rows;
  arma::vec pvec = arma::linspace<arma::vec>(1, p, p);
  double gammapart = 0;
  for (int i=0; i < p; ++i){
    gammapart = gammapart + std::log(std::tgamma((v+1-pvec(i))/2));
  }
  double ldenom = gammapart + 0.5*v*p*log(2)+0.25*p*(p-1)*log2pi;
  arma::mat cholS = chol(S);
  arma::mat cholW = chol(W);
  double halflogdetS = sum(log(cholS.diag()));
  double halflogdetW = sum(log(cholW.diag()));
  arma::mat invW = inv_sympd(W);
  double exptrace = accu(S % invW);
  double lnum = v*halflogdetS - (v+p+1)*halflogdetW-0.5*exptrace;
  double lpdf = lnum - ldenom;
  return lpdf;
}
...
arma::vec betagam_accept_random_c(const arma::vec X,
  ...
  int g = accu(gam1);
  if(change==1){
      proposal_ratio = -proposal_iter + log(T-g) - log(g+1);
    }else{
      proposal_ratio = proposal_iter + log(g) - log(T-g+1);
    }
