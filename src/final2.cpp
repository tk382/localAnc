...
// [[Rcpp::depends("RcppArmadillo")]]
// [[Rcpp::export]]
double gam(int s, double aa, double bb, int T){
  double G = log(std::tgamma(s+aa)) + log(std::tgamma(T+bb-s)) + log(std::tgamma(aa+bb));
  G = G - log(std::tgamma(T+aa+bb)) - log(std::tgamma(aa)) - log(std::tgamma(bb));
  return G;
}
...
arma::vec get_target_c(const arma::vec X,
  ...
  G = log(std::tgamma(s+aa)) + log(std::tgamma(T+bb-s)) + log(std::tgamma(aa+bb));
  G = G - log(std::tgamma(T+aa+bb)) - log(std::tgamma(aa)) - log(std::tgamma(bb));
...
Rcpp::List update_h_c(const double initialh,
                      ...
                      const arma::mat Sig,
                      const arma::vec X,
                      const double prior_h1,
                      const double prior_h2,
                      int T){
  double h1 = initialh;
  ...
  for (int i=1; i<hiter; ++i){
    ...
    // if(h2>0.9){h2 = 1.8-h2;}
    if(h2<prior_h1){h2 = abs(prior_h1*2 - h2);}
    if(h2>prior_h2){h2 = prior_h2*2-h2;}
    arma::uvec ind = find(gam==1);
...
Rcpp::List run2chains_c(const arma::vec X,
                        ...
                        const int hiter   = 50,
                        const int Vbeta   = 1,
                        const double aa = 0.05,
                        const double bb = 0.5,
                        const double prior_h1 = 0.1,
                        const double prior_h2 = 0.9,
